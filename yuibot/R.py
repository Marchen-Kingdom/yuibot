...
import yuibot
from yuibot import logger, util


class ResObj:
    def __init__(self, res_path):
        res_dir = os.path.expanduser(yuibot.config.RES_DIR)
    ...
    def url(self):
        ...
        return urljoin(yuibot.config.RES_URL, pathname2url(self.__path))

    @property
    def path(self):
        """资源文件的路径，供bot内部使用"""
        return os.path.join(yuibot.config.RES_DIR, self.__path)
...
class ResImg(ResObj):
    ...
    def cqcode(self) -> MessageSegment:
        if yuibot.config.RES_PROTOCOL == "http":
            return MessageSegment.image(self.url)
        elif yuibot.config.RES_PROTOCOL == "file":
    ...
    def open(self) -> Image:
        try:
            ...
            yuibot.logger.error(f"缺少图片资源：{self.path}")
            raise
