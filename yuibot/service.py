...
import yuibot
from yuibot import log, priv, trigger
from yuibot.typing import *
...
_service_config_dir = os.path.expanduser("~/.yuibot/service_config/")
...
class Service:
    ...
    储存位置：
    `~/.yuibot/service_config/{ServiceName}.json`
    """
    ...
    def __init__(
        ...
        assert not _re_illegal_char.search(
            name
        ), r'Service name cannot contain character in `\/:*?"<>|.`'
        ...
        self.logger = log.new_logger(name, yuibot.config.DEBUG)
    ...
    async def get_enable_groups(self) -> dict:
        ...
        """
        gl = defaultdict(list)
        for sid in yuibot.get_self_ids():
    ...
    def on_fullmatch(self, word, only_to_me=False) -> Callable:
        ...
        def deco(func) -> Callable:
            @wraps(func)
            async def wrapper(bot: YuiBot, event: CQEvent):
    ...
    def on_rex(
        self, rex: Union[str, re.Pattern], only_to_me=False, normalize=True
    ) -> Callable:
    ...
    def on_command(
        self, name, *, only_to_me=False, deny_tip=None, **kwargs
    ) -> Callable:
            ...
            try:
                for msg in msgs:
                    ...
                    await bot.send_group_msg(
                        self_id=random.choice(selfids), group_id=gid, message=msg
                    )
                l = len(msgs)
                if l:
...
sulogger = log.new_logger("sucmd", yuibot.config.DEBUG)
...
def sucmd(name, force_private=True, **kwargs) -> Callable:
    ...
    def deco(func) -> Callable:
        ...
        async def wrapper(session: CommandSession):
            if session.event.user_id not in yuibot.config.SUPERUSERS:
                return
            if force_private and session.event.detail_type != "private":
                await session.send(
                    "> This command should only be used in private session."
                )
                return
            try:
                return await func(session)
