...
from yuibot import Service, priv, util
from yuibot.config import twitter as cfg
from yuibot.typing import CQEvent

api = TwitterAPI(
    cfg.consumer_key, cfg.consumer_secret, cfg.access_token_key, cfg.access_token_secret
)
sv = Service(
    "twitter-poller", use_priv=priv.SUPERUSER, manage_priv=priv.SUPERUSER, visible=False
)
...
subr_dic = {
    Service(
        "kc-twitter", enable_on_default=False, help_="艦これ官推转发", bundle="kancolle"
    ): ["KanColle_STAFF", "C2_STAFF", "ywwuyi"],
    Service(
        "pcr-twitter", enable_on_default=True, help_="日服Twitter转发", bundle="pcr订阅"
