...
import yuibot
from yuibot import R, Service
from yuibot.typing import *
from yuibot.util import FreqLimiter, concat_pic, pic2b64, silence
...
def render_atk_def_teams(entries, border_pix=5):
    ...
    im = Image.new(
        "RGBA",
        (5 * icon_size + 100, n * (icon_size + border_pix) - border_pix),
        (255, 255, 255, 255),
    )
    ...
    for i, e in enumerate(entries):
        ...
        draw.text(
            (x1 + 16, y1 + 20),
            f"{e['up']}+{e['my_up']}" if e["my_up"] else f"{e['up']}",
            (0, 0, 0, 255),
            font,
        )
        draw.text(
            (x1 + 16, y1 + 40),
            f"{e['down']}+{e['my_down']}" if e["my_down"] else f"{e['down']}",
            (0, 0, 0, 255),
            font,
        )
    return im
...
async def _arena_feedback(bot, ev: CQEvent, action: int):
    ...
    if not qkey:
        await bot.finish(
            ev, f'请发送"点{action_tip}+作业id"，如"点{action_tip}ABCDE"，不分大小写', at_sender=True
        )
...
async def upload(ss: CommandSession):
    ...
    if "pic" not in ss.state:
        ss.state["pic"] = MessageSegment.image(
            pic2b64(
                concat_pic(
                    [
...
async def _(ss: CommandSession):
    ...
    if ss.current_key.endswith("_team"):
        ...
        ss.state[ss.current_key] = [
            chara.fromid(team[i], star[i], equip[i]) for i in range(5)
        ]
