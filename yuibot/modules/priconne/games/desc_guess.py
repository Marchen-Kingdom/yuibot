...
from yuibot import Service
from yuibot.modules.priconne import chara
from yuibot.typing import CQEvent
from yuibot.typing import MessageSegment as Seg
...
ONE_TURN_TIME = 12
TURN_NUMBER = 5
DB_PATH = os.path.expanduser("~/.yuibot/pcr_desc_guess.db")
...
async def description_guess(bot, ev: CQEvent):
    ...
    with gm.start_game(ev.group_id) as game:
        ...
        kws = kws[:TURN_NUMBER]
        await bot.send(
            ev, f"{PREPARE_TIME}秒后每隔{ONE_TURN_TIME}秒我会给出某位角色的一个描述，根据这些描述猜猜她是谁~"
        )
