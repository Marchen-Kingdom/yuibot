import yuibot
from yuibot import Service, priv
from yuibot.typing import CQEvent
from yuibot.util import DailyNumberLimiter
...
async def feedback(bot, ev: CQEvent):
    ...
    coffee = yuibot.config.SUPERUSERS[0]
    text = str(ev.message).strip()
    if not text:
        await bot.send(ev, "请发送来杯咖啡+您要反馈的内容~", at_sender=True)
    else:
        await bot.send_private_msg(
            self_id=ev.self_id, user_id=coffee, message=f"Q{uid}@群{ev.group_id}\n{text}"
        )
