...
DB_PATH = os.path.expanduser("~/.yuibot/clanbattle.db")
...
class SqliteDao(object):
    ...
    def _connect(self):
        ...
        return sqlite3.connect(
            self._dbpath, detect_types=sqlite3.PARSE_DECLTYPES | sqlite3.PARSE_COLNAMES
        )
...
class MemberDao(SqliteDao):
    ...
    def row2item(r):
        return (
            {"uid": r[0], "alt": r[1], "name": r[2], "gid": r[3], "cid": r[4]}
            if r
            else None
        )
        ...
        with self._connect() as conn:
            try:
                ret = conn.execute(
                    ...
                    """.format(
                        self._table, self._columns, cond_str
                    ),
                    cond_tup,
                ).fetchall()
                return [self.row2item(r) for r in ret]
...
class BattleDao(SqliteDao):
    ...
    def find_by(self, uid=None, alt=None, order_by_user=False):
        ...
        cond_tup = []
        order = (
            "round, boss, eid" if not order_by_user else "uid, alt, round, boss, eid"
        )
        if not uid is None:
            cond_str.append("uid=?")
            cond_tup.append(uid)
        ...
        with self._connect() as conn:
            try:
                ret = conn.execute(
                    ...
                    """.format(
                        self._table, self._columns, cond_str, order
                    ),
                    cond_tup,
                ).fetchall()
                return [self.row2item(r) for r in ret]
