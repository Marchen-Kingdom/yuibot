...
# TODO 将预约信息转至数据库
SUBSCRIBE_PATH = os.path.expanduser("~/.yuibot/clanbattle_sub/")
...
async def _do_show_remain(
    ...
    rlist = bm.list_challenge_remain(
        1, datetime.now() - timedelta(days=args.get("D", 0))
    )
...
@cb_cmd(
    "查刀",
    ArgParser(usage="!查刀", arg_dict={"D": ArgHolder(tip="日期差", type=int, default=0)}),
)
