...
class BattleMaster(object):
    ...
    def add_member(self, uid, alt, name, cid):
        return self.memberdao.add(
            {"uid": uid, "alt": alt, "name": name, "gid": self.group, "cid": cid}
        )
    ...
    def filt_challenge_of_day(challenge_list, time, zone_num: int = 8):
        ...
        return list(
            filter(
                lambda challen: day
                == BattleMaster.get_yyyymmdd(challen["time"], zone_num)[2],
                challenge_list,
            )
        )

    def list_challenge_of_day(self, cid, time, zone_num: int = 8):
        return self.filt_challenge_of_day(
            self.list_challenge(cid, time), time, zone_num
        )

    def list_challenge_of_user_of_day(self, uid, alt, time, zone_num: int = 8):
        return self.filt_challenge_of_day(
            self.list_challenge_of_user(uid, alt, time), time, zone_num
        )
    ...
    def stat_damage(self, cid, time):
        ...
        server = clan["server"]
        stat = self.stat_challenge(
            cid, time, only_one_day=False, zone_num=self.get_timezone_num(server)
        )
        ret = []
        ...
        ret = [
            (
                ...
                sum(
                    map(
                        lambda ch: round(
                            self.get_score_rate(ch["round"], ch["boss"], server)
                            * ch["dmg"]
                        ),
                        challens,
                    )
    ...
    def list_challenge_remain(self, cid, time):
        ...
        ret = []
        stat = self.stat_challenge(
            cid, time, only_one_day=True, zone_num=self.get_timezone_num(clan["server"])
        )
        for mem, challens in stat:
