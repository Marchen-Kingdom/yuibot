...
import yuibot
from yuibot.typing import CQEvent, Message, Union
...
async def delete_msg(ev: CQEvent):
    try:
        await yuibot.get_bot().delete_msg(self_id=ev.self_id, message_id=ev.message_id)
    except ActionFailed as e:
        yuibot.logger.error(f"撤回失败 retcode={e.retcode}")
    except Exception as e:
        yuibot.logger.exception(e)
...
async def silence(ev: CQEvent, ban_time, skip_su=True):
    try:
        if skip_su and ev.user_id in yuibot.config.SUPERUSERS:
            return
        await yuibot.get_bot().set_group_ban(
            self_id=ev.self_id,
        ...
        yuibot.logger.error(f"禁言失败 retcode={e.retcode}")
    except Exception as e:
        yuibot.logger.exception(e)
